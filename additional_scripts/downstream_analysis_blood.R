...
c1 <-  ggplot(data_reads, aes(x=Samples, y=Reads, fill=Condition)) +
       ...
       # if (length(unique(data_reads$Protocol > 1))) {facet_grid(. ~ Protocol, scales='free', space = "free", shrink=F)}
...
c2 <-  ggplot(data_reads, aes(x=Condition, y=Reads, fill=Condition)) +
       geom_point(aes(colour = factor(Batch)), alpha = 1, position = "jitter", shape=18, size=1.5) +
       geom_boxplot(aes(fill=Condition), alpha = 0.8, width=0.4) +
       theme_minimal() +
       ...
       labs(title = "Per group boxplot reveiling its group's coverage",
            y = "Groups",
            x = "Reads")
cc <- ggarrange(c1, c2, ncol = 2, nrow = 1)
ggsave(cc, file=paste(main.outdir,"/1.coverage.png",sep=""), width = 12, height = 6, units = "in", dpi = 1200)
...
plot_krona(physeq, output=paste(main.outdir,"/2.krona-samples",sep=""), variable="Samples", trim=T)
# Output overall KRONA visualisation of the classification
plot_krona(physeq, output=paste(main.outdir,"/3.krona-condition",sep=""), variable="Condition", trim=T)
...
ggsave(phyl, file=paste(main.outdir, "/4.taxonomy.phylum.rel-composition.png", sep=""), width = 12, height = 8, units = "in", dpi = 1200)
...
png(paste(main.outdir,"/5.rarefraction_curves.png",sep=""), width = 12, height = 6, units = "in", res = 1200)
rarecurve(t(otu_table(physeq)), col="#11698e", step=50, cex=.5,
...
for (measure in alpha_div) {  print(measure)
    ...
    p2 <- ggplot(microbiome_richness, aes(x=Cond1, y=eval(as.name(measure)))) +
        geom_point(aes(colour = factor(Protocol)), alpha = 1, position = "jitter", shape=18, size=1.5) +
...
ggsave(file=paste(main.outdir, "/6.alpha-diversity.cond1.squareplot.png", sep=""), width = 16, height = 10, units = "in", dpi = 1200)
ggarrange(plotlist=q)
ggsave(file=paste(main.outdir, paste("/7.alpha-diversity.cond1.boxplot.png", sep=""),sep=""), width = 16, height = 10, units = "in", dpi = 1200)
...
d <- micr %>%  gather(key = metric, value = value, c("Shannon","Simpson","Fisher")) %>%
     ...
     theme(panel.border = element_blank(),
           ...
           legend.key = element_rect(colour = "transparent", fill = "white"),
           legend.position="none")
ggsave(d, file=paste(main.outdir, paste("/8.alpha-diversity.cond2.boxplot.png", sep=""),sep=""), width = 12, height = 8, units = "in", dpi = 1200)
...
ggsave(h1, file=paste(main.outdir, "/9.beta-diversity.heatmap.png",sep=""), width = 10, height = 6, units = "in", dpi = 1200)
...
p1 <- plot_ordination(ps_rare, ord.nmds.bray, color="Cond2", title="NMDS plot (Bray-Curtis distance)") +
...
ggsave(pp, file=paste(main.outdir,"/10.ordination.nmds.bray-curtis.png",sep=""), width = 10, height = 8, units = "in", dpi = 1200)
rm(ord.nmds.bray, p1, p2, pp)


# Unweighted Unifrac
ord_nmds_unifrac_unweighted <- ordinate(ps_rare, method="NMDS", distance="unifrac", weighted=FALSE)
g1 <- plot_ordination(ps_rare, ord_nmds_unifrac_unweighted, color="Cond2", title="NMDS plot (Unweighted Unifrac)") + 
                      geom_text(label=sample_data(ps_rare)$Samples, size=2, hjust=.5, vjust=-1.1) +
                      theme_minimal()
...
ggsave(gg, file=paste(main.outdir,"/11.ordination.nmds.unweighted-unifrac.png",sep=""), width = 10, height = 8, units = "in", dpi = 1200)
...
q1 <- plot_ordination(ps_rare, ord_nmds_unifrac_weighted, color="Cond2", title="NMDS plot (Weighted Unifrac)") +
                      geom_text(label=sample_data(ps_rare)$Samples, size=2, hjust=.5, vjust=-1.1) +
...
ggsave(qq, file=paste(main.outdir,"/12.ordination.nmds.weighted-unifrac.png",sep=""), width = 10, height = 8, units = "in", dpi = 1200)
rm(ord_nmds_unifrac_weighted, q1, q2, qq)


# Canonical correspondence analysis
ord_cca <- ordinate(ps_rare, "CCA")
u1 <- plot_ordination(ps_rare, ord_cca, type = "samples", color = "Cond2", title="CCA plot (Bray-Curtis)") + 
    geom_text(label=sample_data(ps_rare)$Samples, size=2, hjust=.5, vjust=-1.1) +
    theme_minimal()
...
ggsave(uu, file=paste(main.outdir,"/13.ordination.cca.bray-curtis.png",sep=""), width = 10, height = 8, units = "in", dpi = 1200)
rm(ord_cca, u1, u2, uu)
...
# Plot the results.
t1 <- plot_tsne_phyloseq(ps_rare, tsne_res_bc, color = 'Cond2', title='t-SNE (Bray-Curtis)') +
                         geom_point(size=3) +
                         geom_text(label=sample_data(ps_rare)$Samples, size=2, hjust=.5, vjust=-1.1) +
                         theme_minimal() 
tsne_res_jsd <- tsne_phyloseq(ps_rare, distance='jsd', perplexity = 8, verbose=0, rng_seed = 3901)
t2 <- plot_tsne_phyloseq(ps_rare, tsne_res_jsd, color = 'Cond2', title='t-SNE (Jensen–Shannon divergence)') +
                         geom_point(size=3) +
                         geom_text(label=sample_data(ps_rare)$Samples, size=2, hjust=.5, vjust=-1.1) +
                         theme_minimal() 

tsne_res_uu <- tsne_phyloseq(ps_rare, distance='unifrac', perplexity = 8, verbose=0, rng_seed = 3901)
t3 <- plot_tsne_phyloseq(ps_rare, tsne_res_uu, color = 'Protocol', title='t-SNE (Unweighted UniFrac)') +
                         geom_point(size=3) +
                         geom_text(label=sample_data(ps_rare)$Samples, size=2, hjust=.5, vjust=-1.1) +
                         theme_minimal() 

tsne_res_wu <- tsne_phyloseq(ps_rare, distance='wunifrac', perplexity = 8, verbose=0, rng_seed = 3901)
t4 <- plot_tsne_phyloseq(ps_rare, tsne_res_wu, color = 'Protocol', title='t-SNE (Weighted UniFrac)') +
                         geom_point(size=3) +
                         geom_text(label=sample_data(ps_rare)$Samples, size=2, hjust=.5, vjust=-1.1) +
                         theme_minimal() 
...
ggsave(tt, file=paste(main.outdir,"/14.ordination.tsne.png",sep=""), width = 10, height = 8, units = "in", dpi = 1200)
...
ggsave(p3, file=paste(main.outdir,"/15.network-analysis.png",sep=""), width = 10, height = 8, units = "in", dpi = 1200)
rm(ig, ig2, p1, p2, p3)
...
ggsave(dep, file=paste(main.outdir,"/16.differential-abundances.analysis.png",sep=""), width = 10, height = 8, units = "in", dpi = 1200)
